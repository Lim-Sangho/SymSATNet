...
class EquivariantLayer(torch.autograd.Function):
    ...
    def forward(ctx, coeff: torch.Tensor, upper: torch.Tensor, basis: torch.Tensor) -> torch.Tensor:
        sub = torch.einsum("b,bij->ij", coeff.cpu(), basis).cuda()
    ...
    def backward(ctx, grad_output: torch.Tensor) -> torch.Tensor:
        ...
        grad_sub = grad_output[1:basis.shape[1]+1, 1:basis.shape[2]+1]
        grad_coeff = torch.einsum("bij,ij->b", basis, grad_sub.cpu()).cuda()
...
class SymSATNet_cpu(torch.nn.Module):

    def __init__(self, n, bases, max_iter=40, eps=1e-4, prox_lam=1e-2):
        super(SymSATNet_cpu, self).__init__()
...
def main():
    ...
    model = SymSATNet_cpu(n, n+1, aux).cuda()
